...
namespace pkpApp.Helpers
{
    public static class CultureHelper
    {
        ...
        "fr-CA",  // first culture is the DEFAULT
        ...
        public static void setCurrentCulture(string newCultureInfo)
        {
            if (!string.IsNullOrWhiteSpace(newCultureInfo))
            {
                //CultureInfo newCulInfo = CultureInfo.CreateSpecificCulture(newCultureInfo);
                //Thread.CurrentThread.CurrentCulture = newCulInfo;
                //Thread.CurrentThread.CurrentUICulture = newCulInfo;

                // Modify current thread's cultures            
                Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo(newCultureInfo);
        ...
        public static string GetDefaultFromCountry()
        {
            var culture = CultureHelper.GetCurrentCulture();
            var cultureUI = CultureHelper.GetCurrentUICulture();
            string ctry;
            if (culture == "fr-CA" && cultureUI == "fr-CA")
            {
                ctry = "124";
            }
            else if (culture == "en-CA" && cultureUI == "en-CA")
            {
                ctry = "124";
            }
            else
            ...
            context.Session["FROM_CTRY_CD"] = ctry;
            return ctry;
        }

        public static string GetDefaultToCountry()
        {
            var culture = CultureHelper.GetCurrentCulture();
            var cultureUI = CultureHelper.GetCurrentUICulture();
            string ctry;
            ...
            HttpContext context = HttpContext.Current;
            context.Session["TO_CTRY_CD"] = ctry;
            return ctry;
        }
